...
class Konfigurasi extends SASUKE_Config {
	...
	public function __construct()
	{
		...
		$this->app = $this->site_m->getSetting();
	}
	...
	public function submit()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			{
				if($this->config_m->addUser($userSetting) == true)
				{
					$status = 1;
					$msg = 'Pengaturan Aplikasi Berhasil. Mengalihkan ke Pengaturan Instansi...';
				}
				else
				{
					$status = 0;
	...
	public function submit_instansi()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			if($this->config_m->instansiSetting($instansi) == true)
			{
				$status = 1;
				$msg = 'Pengaturan Instansi Berhasil. Mengalihkan ke Halaman Login...';
			}
			else
			{
				$status = 0;
